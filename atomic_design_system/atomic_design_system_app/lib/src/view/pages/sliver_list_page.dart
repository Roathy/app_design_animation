      ...
      data: FilledButtonThemeData(
          style: ButtonStyle(
              shape: MaterialStateProperty.all(
                const RoundedRectangleBorder(borderRadius: BorderRadius.only(topLeft: Radius.circular(18))),
              ),
...
class ScrollabeList extends StatelessWidget {
  const ScrollabeList({super.key});
  ...
  Widget build(BuildContext context) {
    return CustomScrollView(
      slivers: [
        ...
        SliverList(delegate: SliverChildBuilderDelegate(childCount: colorList.length, (context, index) => _ListItem(index)))
      ],
    );
  }
}
